...
public:
    ...
    void Fit();
    void Build(int M = -1, int ef_construction = -1, int n_threads = -1, float mult = -1, NeighborSelectingPolicy neighbor_selecting = NeighborSelectingPolicy::HEURISTIC, GraphPostProcessing graph_merging = GraphPostProcessing::SKIP, bool ensure_k = false);

    TreeNode* createCompleteTree(int depth, std::vector<int>& sub_node);

    void SearchByVector(const std::vector<float>& qvec, size_t k, size_t ef_search, size_t alg,
                        std::vector<std::pair<int, float> >& result);

    void SearchById(int id, size_t k, size_t ef_search,
                    std::vector<std::pair<int, float> >& result);

    void SearchByVector_violence(const std::vector<float>& qvec, size_t k, size_t ef_search, std::vector<std::pair<int, float>>& result);
...
private:
    ...
    void SearchAtLayer(const std::vector<float>& qvec, HnswNode* enterpoint, size_t ef,
                       std::priority_queue<FurtherFirst>& result);

    bool IsKeep(HnswNode* enterpoint, HnswNode* tagetpoint, size_t th);

    void SearchById_(int cur_node_id, float cur_dist, const float* query_vec,
                     size_t k, size_t ef_search,
                     std::vector<std::pair<int, float> >& result);

    void SearchById_true(int cur_node_id, float cur_dist, const float* query_vec, unsigned int* visited, std::priority_queue<FurtherFirstNew>& res, std::priority_queue<CloserFirstNew>& candidates,
                     size_t k, size_t ef_search,
                     std::vector<std::pair<int, float> >& result);
...
private:
    ...
    char* model_node0_ = nullptr;
    char* model_tree_node0_ = nullptr;
    size_t data_dim_ = 0;
    long long memory_per_data_ = 0;
    long long memory_per_link_ = 0;
    long long memory_per_node_ = 0;
    long long memory_per_tree_node_ = 0;
    int comparison_times_ = 1;
