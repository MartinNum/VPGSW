...
cdef extern from "n2/hnsw.h" namespace "n2":
    cdef cppclass Hnsw:
        ...
        int GetComparisonTimes() nogil except +
        ...
        void SearchByVector(const vector[float]& qvec, size_t, size_t, size_t, vector[pair[int, float]]& result) nogil except +
        void SearchById(int, size_t, size_t, vector[pair[int, float]]& result) nogil except +
        void SearchByVector_violence(const vector[float]& qvec, size_t, size_t, vector[pair[int, float]]& result) nogil except +
...
cdef class _HnswIndex:
    ...
    def get_comparison_times(self):
        return self.obj.GetComparisonTimes()
    ...
    def search_by_vector(self, _v, _k, _ef_search, _alg):
        cdef vector[float] v = _v
        cdef size_t k = _k
        cdef size_t ef_search = _ef_search
        cdef size_t alg = _alg
        cdef vector[pair[int, float]] ret
        with nogil:
            self.obj.SearchByVector(v, k, ef_search, alg, ret)
        return ret
    ...
    def search_by_vector_violence(self, _v, _k, _ef_search):
        cdef vector[float] v = _v
        cdef size_t k = _k
        cdef size_t ef_search = _ef_search
        cdef vector[pair[int, float]] ret
        with nogil:
            self.obj.SearchByVector_violence(v, k, ef_search, ret)
        return ret
...
class HnswIndex(object):
    ...
    def get_comparison_times(self):
        return self.model.get_comparison_times()
    ...
    def build(self, m=None, ef_construction=None, n_threads=None, mult=None, neighbor_selecting=None, graph_merging=None):
        """
    ...
    def search_by_vector(self, v, k, ef_search=-1, alg=-1, include_distances=False):
        """
        ...
        :param alg: If you set this argument to -1,it will use SearchById_,If you set this argument to 1,it will use SearchById_true.
        ...
        if alg == -1:
            alg = 1;
        ret = self.model.search_by_vector(v, k, ef_search, alg)
    ...
    def search_by_vector_violence(self, v, k, ef_search=-1, include_distances=False):
        """
        Returns k nearest items by vector.
        ...
        if ef_search == -1:
            ef_search = k * 10
        ret = self.model.search_by_vector_violence(v, k, ef_search)
        if include_distances:
            return ret
        else:
            return [k for k, _ in ret]
